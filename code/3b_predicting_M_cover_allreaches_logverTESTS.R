...
# LOGGED VERSION!!!! (NOW A TESTING SCRIPT :)
...
preds_cover_trunc_for_log <- function(params, y, covar) {
  ...
  preds[,1] <- 0.005 # assign first values to zero (hard-coded because we start all w/ zero)
...
# when cover = 0, add 0.005 (so model can increase as we multiply by this value)
data <- data %>%
  mutate(resp_AC_cover_norm = case_when(resp_AC_cover_norm == 0 ~ 0.005,
                                        TRUE ~ resp_AC_cover_norm),
         future_AC_cover_norm = case_when(future_AC_cover_norm == 0 ~ 0.005,
...
for(j in 2:length(predictions)) {
  ...
  for(i in 1:length(training_sites)) {
    ...
    # save STAN model
    saveRDS(model, paste("./data/predictive_models/log_tests/starting_value_0.005/AC_cover_models/", model_name, 
  ...
  write.csv(rhats, paste("./data/predictive_models/log_tests/starting_value_0.005/AC_cover_models/model_attributes/",
                         model_name, "_rhats.csv", sep = ""), row.names = TRUE)
  write.csv(param_est,  paste("./data/predictive_models/log_tests/starting_value_0.005/AC_cover_models/model_attributes/",
...
# saving nRMSE table
write.csv(nrmse %>% na.omit(), "./data/predictive_models/log_tests/starting_value_0.005/nrmse_AC_cover.csv",
          row.names = FALSE)
...
write.csv(final_predictions, "./data/predictive_models/log_tests/starting_value_0.005/predictions_AC_cover.csv",
          row.names = FALSE)
