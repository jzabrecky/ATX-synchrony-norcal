...
# DEBUGGING: just do j = 6 which is ecohydrological; don't run for loop
j <- 6
for(j in 2:length(predictions)) {
  ...
  for(i in 1:length(training_sites)) {
    ...
    model <- stan(file = "./code/model_STAN_files/predicting_cover_option2.stan", 
                 data = mod_data,
                 chains = 3, iter = 10000, warmup = 5000, 
                 control = list(adapt_delta = 0.95, max_treedepth = 12))
    # save STAN model
    saveRDS(model, paste("./data/predictive_models/option2/debug_trial/M_cover_models/", model_name, 
    ...
    preds_matrix <- preds_cover_option2(params = params,
                               y = predictions[[j]][[i]],
  ...
  write.csv(rhats, paste("./data/predictive_models/option2/debug_trial/M_cover_models/model_attributes/",
                         model_name, "_rhats.csv", sep = ""), row.names = TRUE)
  write.csv(param_est, paste("./data/predictive_models/option2/debug_trial/M_cover_models/model_attributes/",
...
# saving nRMSE table
write.csv(nrmse %>% na.omit(), "./data/predictive_models/option2/debug_trial/nrmse_M_cover.csv",
          row.names = FALSE)
...
write.csv(final_predictions, "./data/predictive_models/option2/debug_trial/predictions_M_cover.csv",
          row.names = FALSE)
