...
# note that null is currently calculated wrong and needs to be exponentiated!


## test! adding 0.1 to cover values when 0
...
raw_data <- read.csv("./data/predictive_models/inputs.csv") %>% 
  select(field_date, site_reach, resp_AC_cover_norm, temp_C,
...
# when cover = 0, add 0.1, then log all values
data <- data %>%
  mutate(resp_AC_cover_norm = case_when(resp_AC_cover_norm == 0 ~ 0.1,
...
data$future_AC_cover_norm <- c(data$resp_AC_cover_norm[-1], NA)
...
check <- data %>% 
  select(field_date, site_reach, resp_AC_cover_norm, DIN_mg_N_L, future_AC_cover_norm)
view(check)
...
for(j in 2:length(predictions)) {
  ...
  for(i in 1:length(training_sites)) {
    ...
    # save STAN model
    saveRDS(model, paste("./data/predictive_models/add_0.1_withtruncation/AC_cover_models/", model_name, 
  ...
  write.csv(rhats, paste("./data/predictive_models/add_0.1_withtruncation/AC_cover_models/model_attributes/",
                         model_name, "_rhats.csv", sep = ""), row.names = TRUE)
  write.csv(param_est, paste("./data/predictive_models/add_0.1_withtruncation/AC_cover_models/model_attributes/",
...
# saving nRMSE table
write.csv(nrmse %>% na.omit(), "./data/predictive_models/add_0.1_withtruncation/nrmse_AC_cover.csv",
          row.names = FALSE)
...
write.csv(final_predictions, "./data/predictive_models/add_0.1_withtruncation/predictions_AC_cover.csv",
          row.names = FALSE)
