...
combined <- left_join(metadata, anatoxins_processed, by = "ESF_ID") %>% 
  filter(sample_type != "NT") %>%  # only calculating targeted mat samples ATX her
  dplyr::select(!ESF_ID) %>% 
...
# save csv
write.csv(combined, './data/EDI_data_package/target_sample_anatoxins.csv', row.names = FALSE)
