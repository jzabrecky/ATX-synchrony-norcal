...
# loading in data and select only columns we care about and remove SFE-M-4 (odd behavior reach)
...
for(j in 2:length(predictions)) {
  ...
  param_est <- data.frame(matrix(NA, nrow = ncol(covariates[[j]]$training[[1]]) 
                                 ...
                                 + 3, ncol = 4)) # plus 3 includes b0, sigma, & lp
  ...
  for(i in 1:length(training_sites)) {
    ...
    # save STAN model
    saveRDS(model, paste("./data/predictive_models/M_cover_models_oddreachout/", model_name, 
  ...
  write.csv(rhats, paste("./data/predictive_models/M_cover_models_oddreachout/model_attributes/",
                         model_name, "_rhats.csv", sep = ""), row.names = TRUE)
  write.csv(param_est,  paste("./data/predictive_models/M_cover_models_oddreachout/model_attributes/",
...
# saving nRMSE table
write.csv(nrmse %>% na.omit(), "./data/predictive_models/nrmse_M_cover_oddreachout.csv",
          row.names = FALSE)
...
write.csv(final_predictions, "./data/predictive_models/predictions_M_cover_oddreachout.csv",
          row.names = FALSE)
