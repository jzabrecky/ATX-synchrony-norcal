#### predicting to determine process uncertainty in models predicting Anabaena/Cylindrospermum anatoxins
### Jordan Zabrecky
## last edited: 01.13.2026

# This script uses previously built models (see script 3e) to predict 
# Anabaena/Cylindrospermum anatoxins WITHOUT incorporating parameter uncertainty (by holding
...
raw_data <- read.csv("./data/predictive_models/inputs.csv") %>% 
  select(field_date, site_reach, resp_AC_atx_norm, future_AC_atx_norm, temp_C,
         discharge_m3_s, DIN_mg_N_L, oPhos_ug_P_L, cond_uS_cm,
         GPP_median_tofourdaysprior, anabaena_cylindrospermum)
...
# save null NMRSE
write.csv(NRMSE, "./data/predictive_models/process_uncertainty/AC_atx_models/NRMSE_vectors/null.csv", row.names = FALSE)
...
covariates[[3]] <- make_covariates(c("temp_C", "discharge_m3_s", "anabaena_cylindrospermum"))
...
covariates[[5]] <- make_covariates(c("DIN_mg_N_L", "oPhos_ug_P_L", "cond_uS_cm", "anabaena_cylindrospermum"))
...
covariates[[7]] <- make_covariates(c("GPP_median_tofourdaysprior", "anabaena_cylindrospermum"))
...
covariates[[9]] <- make_covariates(c("temp_C", "discharge_m3_s",
                                     "DIN_mg_N_L", "oPhos_ug_P_L",
                                     "cond_uS_cm", "anabaena_cylindrospermum"))
...
for(j in 2:length(predictions)) {
  ...
  for(i in 1:length(training_sites)) {
    ...
    mod_data = list(N = nrow(training_sites[[i]]),
                    ...
                    future = training_sites[[i]]$future_AC_atx_norm,
                    covar = as.matrix(covariates[[j]]$training[[i]]))
    # read in model previously built (script 3e)
    ...
    write.csv(preds_matrix, paste("./data/predictive_models/process_uncertainty/AC_atx_models/pred_matrices/",
    ...
    # save NRMSE vector
    write.csv(NRMSE, paste("./data/predictive_models/process_uncertainty/AC_atx_models/NRMSE_vectors/",
...
write.csv(final_predictions, "./data/predictive_models/process_uncertainty/predictions_AC_atx.csv",
          row.names = FALSE)
