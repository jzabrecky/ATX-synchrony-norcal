...
preds_cover_trunc_for_log <- function(params, y, covar) {
  ...
  preds[,1] <- 0.5 # assign first values to zero (hard-coded because we start all w/ zero)
...
data <- data %>%
  mutate(resp_M_cover_norm = case_when(resp_M_cover_norm == 0 ~ 0.6,
                                       TRUE ~ resp_M_cover_norm),
         future_M_cover_norm = case_when(future_M_cover_norm == 0 ~ 0.6,
