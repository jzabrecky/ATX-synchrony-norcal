#### models to predict cover (no autoregressive, truncated norm)
### Jordan Zabrecky
...
# to see how avoiding using that autoregressive term changes our predictions
...
physical_param_est <- data.frame(matrix(NA, nrow = 5, ncol = 5))
physical_rhats <- data.frame(matrix(NA, nrow = 5, ncol = 5))
...
for(i in 1:length(training_sites)) {
  ...
  mod_data <- list(N = nrow(training_sites[[i]]),
                   ...
                   covar = covariates)
  # run STAN model
  model <- stan(file = "./code/model_STAN_files/predicting_ATX.stan", 
                data = mod_data,
                chains = 3, iter = 2000, warmup = 1000, 
                control = list(adapt_delta = 0.95))
  # save STAN model
  saveRDS(model, paste("./data/predictive_models/no_autoregressive/M_cover_models/physical_", 
  ...
  preds_matrix <- preds_atx(params = params,
...
write.csv(physical_rhats,
          "./data/predictive_models/no_autoregressive/M_cover_models/model_attributes/physical_rhats.csv",
          row.names = TRUE)

# looking at how parameter estimates change across all models
view(physical_param_est)
write.csv(physical_param_est,
          "./data/predictive_models/no_autoregressive/M_cover_models/model_attributes/physical_param_est.csv",
          row.names = TRUE)
...
# empty list to save parameter estimates for each model
# and parameter r-hats for each model
chemical_param_est <- data.frame(matrix(NA, nrow = 6, ncol = 5))
chemical_rhats <- data.frame(matrix(NA, nrow = 6, ncol = 5))
...
for(i in 1:length(training_sites)) {
  ...
  model <- stan(file = "./code/model_STAN_files/predicting_ATX.stan", 
                ...
                control = list(adapt_delta = 0.95))
  # save STAN model
  saveRDS(model, paste("./data/predictive_models/no_autoregressive/M_cover_models/chemical_", 
                       names(test_sites)[i], sep = ""))
  # extract parameters
  params <- rstan::extract(model, c("sigma", "b0",  "b"))
  # add mean parameter estimates to dataframe
...
write.csv(chemical_param_est,
          "./data/predictive_models/no_autoregressive/M_cover_models/model_attributes/chemical_rhats.csv",
          row.names = TRUE)

# looking at how parameter estimates change across all models
view(chemical_param_est)
write.csv(chemical_param_est,
          "./data/predictive_models/no_autoregressive/M_cover_models/model_attributes/chemical_param_est.csv",
          row.names = TRUE)
...
# empty list to save parameter estimates for each model
# and parameter r-hats for each model
biological_param_est <- data.frame(matrix(NA, nrow = 4, ncol = 5))
biological_rhats <- data.frame(matrix(NA, nrow = 4, ncol = 5))
...
for(i in 1:length(training_sites)) {
  ...
  model <- stan(file = "./code/model_STAN_files/predicting_ATX.stan", 
                ...
                control = list(adapt_delta = 0.95))
  # save STAN model
  saveRDS(model, paste("./data/predictive_models/no_autoregressive/M_cover_models/biological_", 
                       names(test_sites)[i], sep = ""))
  # extract parameters
  params <- rstan::extract(model, c("sigma", "b0",  "b"))
  # add mean parameter estimates to dataframe
...
write.csv(biological_rhats,
          "./data/predictive_models/no_autoregressive/M_cover_models/model_attributes/biological_rhats.csv",
          row.names = TRUE)

# looking at how parameter estimates change across all models
view(biological_param_est)
write.csv(biological_param_est,
          "./data/predictive_models/no_autoregressive/M_cover_models/model_attributes/biological_param_est.csv",
          row.names = TRUE)
...
# empty list to save parameter estimates for each model
# and parameter r-hats for each model
physicochemical_param_est <- data.frame(matrix(NA, nrow = 8, ncol = 5))
physicochemical_rhats <- data.frame(matrix(NA, nrow = 8, ncol = 5))
...
for(i in 1:length(training_sites)) {
  ...
  model <- stan(file = "./code/model_STAN_files/predicting_ATX.stan", 
                ...
                control = list(adapt_delta = 0.95))
  # save STAN model
  saveRDS(model, paste("./data/predictive_models/no_autoregressive/M_cover_models/physicochemical_", 
                       names(test_sites)[i], sep = ""))
  # extract parameters
  params <- rstan::extract(model, c("sigma", "b0",  "b"))
  # add mean parameter estimates to dataframe
...
write.csv(physicochemical_rhats,
          "./data/predictive_models/no_autoregressive/M_cover_models/model_attributes/physicochemical_rhats.csv",
          row.names = TRUE)

# looking at how parameter estimates change across all models
view(physicochemical_param_est)
write.csv(physicochemical_param_est,
          "./data/predictive_models/no_autoregressive/M_cover_models/model_attributes/physicochemical_param_est.csv",
          row.names = TRUE)
...
# empty list to save parameter estimates for each model
# and parameter r-hats for each model
ecohydrological_param_est <- data.frame(matrix(NA, nrow = 6, ncol = 5))
ecohydrological_rhats <- data.frame(matrix(NA, nrow = 6, ncol = 5))
...
for(i in 1:length(training_sites)) {
  ...
  model <- stan(file = "./code/model_STAN_files/predicting_ATX.stan", 
                ...
                control = list(adapt_delta = 0.95))
  # save STAN model
  saveRDS(model, paste("./data/predictive_models/no_autoregressive/M_cover_models/ecohydrological_", 
                       names(test_sites)[i], sep = ""))
  # extract parameters
  params <- rstan::extract(model, c("sigma", "b0",  "b"))
  # add mean parameter estimates to dataframe
...
write.csv(ecohydrological_rhats,
          "./data/predictive_models/no_autoregressive/M_cover_models/model_attributes/ecohydrological_rhats.csv",
          row.names = TRUE)

# looking at how parameter estimates change across all models
view(ecohydrological_param_est)
write.csv(ecohydrological_param_est,
          "./data/predictive_models/no_autoregressive/M_cover_models/model_attributes/ecohydrological_param_est.csv",
          row.names = TRUE)
...
# empty list to save parameter estimates for each model
# and parameter r-hats for each model
biochemical_param_est <- data.frame(matrix(NA, nrow = 7, ncol = 5))
biochemical_rhats <- data.frame(matrix(NA, nrow = 7, ncol = 5))
...
for(i in 1:length(training_sites)) {
  ...
  model <- stan(file = "./code/model_STAN_files/predicting_ATX.stan", 
                ...
                control = list(adapt_delta = 0.95))
  # save STAN model
  saveRDS(model, paste("./data/predictive_models/no_autoregressive/M_cover_models/biochemical_", 
                       names(test_sites)[i], sep = ""))
  # extract parameters
  params <- rstan::extract(model, c("sigma", "b0",  "b"))
  # add mean parameter estimates to dataframe
...
write.csv(biochemical_rhats,
          "./data/predictive_models/no_autoregressive/M_cover_models/model_attributes/biochemical_rhats.csv",
          row.names = TRUE)
...
# empty list to save parameter estimates for each model
# and parameter r-hats for each model
all_param_est <- data.frame(matrix(NA, nrow = 9, ncol = 5))
all_rhats <- data.frame(matrix(NA, nrow = 9, ncol = 5))
...
for(i in 1:length(training_sites)) {
  ...
  model <- stan(file = "./code/model_STAN_files/predicting_ATX.stan", 
                ...
                control = list(adapt_delta = 0.95))
  # save STAN model
  saveRDS(model, paste("./data/predictive_models/no_autoregressive/M_cover_models/all_", 
                       names(test_sites)[i], sep = ""))
  # extract parameters
  params <- rstan::extract(model, c("sigma", "b0",  "b"))
  # add mean parameter estimates to dataframe
...
write.csv(all_rhats,
          "./data/predictive_models/no_autoregressive/M_cover_models/model_attributes/all_rhats.csv",
          row.names = TRUE)

# looking at how parameter estimates change across all models
view(all_param_est)
write.csv(all_param_est,
          "./data/predictive_models/no_autoregressive/M_cover_models/model_attributes/all_param_est.csv",
          row.names = TRUE)

#### (5) Saving outputs ####

# saving nRMSE table
write.csv(nrmse %>% na.omit(), "./data/predictive_models/no_autoregressive/nrmse_M_cover.csv",
          row.names = FALSE)
...
write.csv(final_predictions, "./data/predictive_models/no_autoregressive/predictions_M_cover.csv",
          row.names = FALSE)
