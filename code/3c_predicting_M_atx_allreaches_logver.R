...
preds_atx_trunc_for_log <- function(params, y, covar) {
  n.pred <- nrow(y) # includes initial day where we used 0
  preds <- matrix(NA, length(params$sigma), n.pred) # empty prediction matrix
  preds[,1] <- 0.6 # assign first values to zero (hard-coded because we start all w/ zero)
...
data <- data %>%
  mutate(resp_M_atx_norm = case_when(resp_M_atx_norm == 0 ~ 0.6,
                                        TRUE ~ resp_M_atx_norm),
         future_M_atx_norm = case_when(future_M_atx_norm == 0 ~ 0.6,
                                          TRUE ~ future_M_atx_norm)) %>% 
  # log values
  mutate(resp_M_atx_norm = log(resp_M_atx_norm),
         future_M_atx_norm = log(future_M_atx_norm))
...
mean_atx <- mean(exp(data$future_M_atx_norm)) # need to exponential for % on 0,100 scale
...
for(i in 1:length(test_sites)) {
  ...
  new$mean <- calc_nRMSE(predictions$null[[i]]$mean[-1], test_sites[[i]]$future_M_atx_norm,
                         # predictions values are on 0-100% scale so also need to exponentiate normalization
                         max(exp(test_sites[[i]]$future_M_atx_norm)), min(exp(test_sites[[i]]$future_M_atx_norm)))
  new$ci_lower <- calc_nRMSE(predictions$null[[i]]$ci_lower[-1], test_sites[[i]]$future_M_atx_norm,
                             max(exp(test_sites[[i]]$future_M_atx_norm)), min(exp(test_sites[[i]]$future_M_atx_norm)))
  new$ci_upper <- calc_nRMSE(predictions$null[[i]]$ci_upper[-1], test_sites[[i]]$future_M_atx_norm,
                             max(exp(test_sites[[i]]$future_M_atx_norm)), min(exp(test_sites[[i]]$future_M_atx_norm)))
...
for(j in 2:length(predictions)) {
  ...
  for(i in 1:length(training_sites)) {
    ...
    # save STAN model
    saveRDS(model, paste("./data/predictive_models/log_ver/M_atx_models/", model_name, 
    ...
    preds_matrix <- preds_atx_trunc_for_log(params = params,
                                y = predictions[[j]][[i]],
    ...
    nrmse <- rbind(nrmse, nRMSE_summary(preds_matrix, exp(test_sites[[i]]$future_M_atx_norm),
                                        site_reach_name = names(test_sites)[i],
                                        model_name = model_name))
  }
  
  # save rhats and mean parameter estimates
  write.csv(rhats, paste("./data/predictive_models/log_ver/M_atx_models/model_attributes/",
                         model_name, "_rhats.csv", sep = ""), row.names = TRUE)
  write.csv(param_est,  paste("./data/predictive_models/log_ver/M_atx_models/model_attributes/",
...
# saving nRMSE table
write.csv(nrmse %>% na.omit(), "./data/predictive_models/log_ver/nrmse_M_atx.csv",
          row.names = FALSE)
...
write.csv(final_predictions, "./data/predictive_models/log_ver/predictions_M_atx.csv",
          row.names = FALSE)
