...
data <- data %>%
  mutate(resp_M_cover_norm = case_when(resp_M_cover_norm == 0 ~ 0.1,
...
for(j in 2:length(predictions)) {
  ...
  for(i in 1:length(training_sites)) {
    ...
    # save STAN model
    saveRDS(model, paste("./data/predictive_models/add_0.1_withtruncation/M_cover_models/", model_name, 
  ...
  write.csv(rhats, paste("./data/predictive_models/add_0.1_withtruncation/M_cover_models/model_attributes/",
                         model_name, "_rhats.csv", sep = ""), row.names = TRUE)
  write.csv(param_est, paste("./data/predictive_models/add_0.1_withtruncation/M_cover_models/model_attributes/",
...
# saving nRMSE table
write.csv(nrmse %>% na.omit(), "./data/predictive_models/add_0.1_withtruncation/nrmse_M_cover.csv",
          row.names = FALSE)
...
write.csv(final_predictions, "./data/predictive_models/add_0.1_withtruncation/predictions_M_cover.csv",
          row.names = FALSE)
