data {
  ...
  array[N] real future; // response/percent cover we are predicting
  array[N] real present; // cover at current timestep (prior to one we are predicting)
...
model {
  for(i in 1:N) {
   future[i] ~ normal(b0 + b1*present[i] + covar[i]*b, sigma);
  }
