data {
  ...
  array[N] real<lower=0, upper=1> future; // response/percent cover we are predicting
  array[N] real<lower=0, upper=1> present; // cover at current timestep (prior to one we are predicting)
...
model {
  for(i in 1:N) {
   future[i] ~ normal(b0 + b1*present[i] + covar[i]*b, sigma) T[0,1];
  }
