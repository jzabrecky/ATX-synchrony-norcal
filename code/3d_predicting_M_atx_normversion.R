...
# This script builds models to predict anatoxins of benthic Microcoleus
# as determined by measuring sample anatoxin concentrations.
# Each model is built using 4 of 
# 5 reaches, then the model is tested using the withheld fifth reach

# note: has autoregressive term!!!!
...
for(i in 1:length(test_sites)) {
  new <- data.frame(site_reach = names(test_sites)[i],
                    model = "null")
  new$mean <- calc_nRMSE(predictions$null[[i]]$mean[-1], test_sites[[i]]$resp_M_atx_norm,
                         max(test_sites[[i]]$resp_M_atx_norm), min(test_sites[[i]]$resp_M_atx_norm))
  new$ci_lower <- calc_nRMSE(predictions$null[[i]]$ci_lower[-1], test_sites[[i]]$resp_M_atx_norm,
                             max(test_sites[[i]]$resp_M_atx_norm), min(test_sites[[i]]$resp_M_atx_norm))
  new$ci_upper <- calc_nRMSE(predictions$null[[i]]$ci_upper[-1], test_sites[[i]]$resp_M_atx_norm,
                             max(test_sites[[i]]$resp_M_atx_norm), min(test_sites[[i]]$resp_M_atx_norm))
...
for(i in 1:length(training_sites)) {
  ...
  # save STAN model
  saveRDS(model, paste("./data/predictive_models/M_atx_normaltest_w_autoregressive/M_atx_models/physical_", 
...
write.csv(physical_rhats,
          "./data/predictive_models/M_atx_normaltest_w_autoregressive/M_atx_models/model_attributes/physical_rhats.csv",
          row.names = TRUE)

# looking at how parameter estimates change across all models
view(physical_param_est)
write.csv(physical_param_est,
          "./data/predictive_models/M_atx_normaltest_w_autoregressive/M_atx_models/model_attributes/physical_param_est.csv",
          row.names = TRUE)
...
for(i in 1:length(training_sites)) {
  ...
  model <- stan(file = "./code/model_STAN_files/predicting_cover_normalnottruncated.stan", 
                ...
                control = list(adapt_delta = 0.95))
  # save STAN model
  saveRDS(model, paste("./data/predictive_models/M_atx_normaltest_w_autoregressive/M_atx_models/chemical_", 
                       names(test_sites)[i], sep = ""))
  # extract parameters
  params <- rstan::extract(model, c("sigma", "b0", "b1", "b"))
  # add mean parameter estimates to dataframe
...
write.csv(chemical_param_est,
          "./data/predictive_models/M_atx_normaltest_w_autoregressive/M_atx_models/model_attributes/chemical_rhats.csv",
          row.names = TRUE)

# looking at how parameter estimates change across all models
view(chemical_param_est)
write.csv(chemical_param_est,
          "./data/predictive_models/M_atx_normaltest_w_autoregressive/M_atx_models/model_attributes/chemical_param_est.csv",
          row.names = TRUE)
...
for(i in 1:length(training_sites)) {
  ...
  model <- stan(file = "./code/model_STAN_files/predicting_cover_normalnottruncated.stan", 
                ...
                control = list(adapt_delta = 0.95))
  # save STAN model
  saveRDS(model, paste("./data/predictive_models/M_atx_normaltest_w_autoregressive/M_atx_models/biological_", 
                       names(test_sites)[i], sep = ""))
  # extract parameters
  params <- rstan::extract(model, c("sigma", "b0", "b1", "b"))
  # add mean parameter estimates to dataframe
...
write.csv(biological_rhats,
          "./data/predictive_models/M_atx_normaltest_w_autoregressive/M_atx_models/model_attributes/biological_rhats.csv",
          row.names = TRUE)

# looking at how parameter estimates change across all models
view(biological_param_est)
write.csv(biological_param_est,
          "./data/predictive_models/M_atx_normaltest_w_autoregressive/M_atx_models/model_attributes/biological_param_est.csv",
          row.names = TRUE)
...
for(i in 1:length(training_sites)) {
  ...
  model <- stan(file = "./code/model_STAN_files/predicting_cover_normalnottruncated.stan", 
                ...
                control = list(adapt_delta = 0.95))
  # save STAN model
  saveRDS(model, paste("./data/predictive_models/M_atx_normaltest_w_autoregressive/M_atx_models/physicochemical_", 
                       names(test_sites)[i], sep = ""))
  # extract parameters
  params <- rstan::extract(model, c("sigma", "b0", "b1", "b"))
  # add mean parameter estimates to dataframe
...
write.csv(physicochemical_rhats,
          "./data/predictive_models/M_atx_normaltest_w_autoregressive/M_atx_models/model_attributes/physicochemical_rhats.csv",
          row.names = TRUE)

# looking at how parameter estimates change across all models
view(physicochemical_param_est)
write.csv(physicochemical_param_est,
          "./data/predictive_models/M_atx_normaltest_w_autoregressive/M_atx_models/model_attributes/physicochemical_param_est.csv",
          row.names = TRUE)
...
for(i in 1:length(training_sites)) {
  ...
  model <- stan(file = "./code/model_STAN_files/predicting_cover_normalnottruncated.stan", 
                ...
                control = list(adapt_delta = 0.95))
  # save STAN model
  saveRDS(model, paste("./data/predictive_models/M_atx_normaltest_w_autoregressive/M_atx_models/ecohydrological_", 
                       names(test_sites)[i], sep = ""))
  # extract parameters
  params <- rstan::extract(model, c("sigma", "b0", "b1", "b"))
  # add mean parameter estimates to dataframe
...
write.csv(ecohydrological_rhats,
          "./data/predictive_models/M_atx_normaltest_w_autoregressive/M_atx_models/model_attributes/ecohydrological_rhats.csv",
          row.names = TRUE)

# looking at how parameter estimates change across all models
view(ecohydrological_param_est)
write.csv(ecohydrological_param_est,
          "./data/predictive_models/M_atx_normaltest_w_autoregressive/M_atx_models/model_attributes/ecohydrological_param_est.csv",
          row.names = TRUE)
...
for(i in 1:length(training_sites)) {
  ...
  model <- stan(file = "./code/model_STAN_files/predicting_cover_normalnottruncated.stan", 
                ...
                control = list(adapt_delta = 0.95))
  # save STAN model
  saveRDS(model, paste("./data/predictive_models/M_atx_normaltest_w_autoregressive/M_atx_models/biochemical_", 
                       names(test_sites)[i], sep = ""))
  # extract parameters
  params <- rstan::extract(model, c("sigma", "b0", "b1", "b"))
  # add mean parameter estimates to dataframe
...
write.csv(biochemical_rhats,
          "./data/predictive_models/M_atx_normaltest_w_autoregressive/M_atx_models/model_attributes/biochemical_rhats.csv",
          row.names = TRUE)
...
for(i in 1:length(training_sites)) {
  ...
  model <- stan(file = "./code/model_STAN_files/predicting_cover_normalnottruncated.stan", 
                ...
                control = list(adapt_delta = 0.95))
  # save STAN model
  saveRDS(model, paste("./data/predictive_models/M_atx_normaltest_w_autoregressive/M_atx_models/all_", 
                       names(test_sites)[i], sep = ""))
  # extract parameters
  params <- rstan::extract(model, c("sigma", "b0", "b1", "b"))
  # add mean parameter estimates to dataframe
...
write.csv(all_rhats,
          "./data/predictive_models/M_atx_normaltest_w_autoregressive/M_atx_models/model_attributes/all_rhats.csv",
          row.names = TRUE)

# looking at how parameter estimates change across all models
view(all_param_est)
write.csv(all_param_est,
          "./data/predictive_models/M_atx_normaltest_w_autoregressive/M_atx_models/model_attributes/all_param_est.csv",
          row.names = TRUE)

#### (5) Saving outputs ####

# saving nRMSE table
write.csv(nrmse %>% na.omit(), "./data/predictive_models/M_atx_normaltest_w_autoregressive/nrmse_M_atx.csv",
          row.names = FALSE)
...
write.csv(final_predictions, "./data/predictive_models/M_atx_normaltest_w_autoregressive/predictions_M_atx.csv",
          row.names = FALSE)
