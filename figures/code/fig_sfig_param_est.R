...
# all for all plots
## microcoleus cover
m_cover <- ggplot(data = param_est_list[[4]] %>% filter(model == "all"), aes(y = parameters_f)) +
  geom_hline(yintercept = (1:5)+0.5, linetype = "dashed", color = "gray") +
  geom_point(aes(x = mean, color = site_reach), position = position_dodge(width = 1)) +
  geom_errorbar(aes(xmin = ci_lower, xmax = ci_upper, color = site_reach),
                position = position_dodge(width = 1)) +
...
## anabaena/cylindrospermum cover
ac_cover <- ggplot(data = param_est_list[[2]] %>% filter(model == "all"), aes(y = parameters_f)) +
  geom_hline(yintercept = (1:5)+0.5, linetype = "dashed", color = "gray") +
  geom_point(aes(x = mean, color = site_reach), position = position_dodge(width = 1)) +
  geom_errorbar(aes(xmin = ci_lower, xmax = ci_upper, color = site_reach),
                position = position_dodge(width = 1)) +
  ...
  theme(strip.background = element_blank()) + # get rid of gray background for facet title
  theme(legend.position = "none") +
  labs(x = "posterior estimates", y = "", title = "*Anabaena/Cylindrospermum* Cover")

## microcoleus ATX
m_atx <- ggplot(data = param_est_list[[3]] %>% filter(model == "all"), aes(y = parameters_f)) +
  geom_hline(yintercept = (1:5)+0.5, linetype = "dashed", color = "gray") +
  geom_point(aes(x = mean, color = site_reach), position = position_dodge(width = 1)) +
  geom_errorbar(aes(xmin = ci_lower, xmax = ci_upper, color = site_reach),
                position = position_dodge(width = 1)) +
  ...
  scale_color_manual(values = palette) +
  scale_x_continuous(trans = pseudolog10_trans,
                     breaks = c(-250, -10, 0, 10, 250)) +
  theme(strip.background = element_blank()) + # get rid of gray background for facet title
  theme(legend.position = "none") +
  labs(x = "posterior estimates", y = "", title = "*Microcoleus* Mat Anatoxins")

m_atx_w_c <- ggplot(data = param_est_list[[3]] %>% filter(model == "all_w_cover"), aes(y = parameters_f)) +
  geom_hline(yintercept = (1:5)+0.5, linetype = "dashed", color = "gray") +
  geom_point(aes(x = mean, color = site_reach), position = position_dodge(width = 1)) +
  geom_errorbar(aes(xmin = ci_lower, xmax = ci_upper, color = site_reach),
                position = position_dodge(width = 1)) +
...
## Anabaena/Cylindrospermum ATX
ac_atx <- ggplot(data = param_est_list[[1]] %>% filter(model == "all"), aes(y = parameters_f)) +
  geom_hline(yintercept = (1:5)+0.5, linetype = "dashed", color = "gray") +
  geom_point(aes(x = mean, color = site_reach), position = position_dodge(width = 1)) +
  geom_errorbar(aes(xmin = ci_lower, xmax = ci_upper, color = site_reach),
                position = position_dodge(width = 1)) +
  ...
  scale_color_manual(values = palette) +
  scale_x_continuous(trans = pseudolog10_trans,
                     breaks = c(-500, -100, -10, 0, 10, 100, 500)) +
  theme(strip.background = element_blank()) + # get rid of gray background for facet title
  theme(legend.position = "none") +
  labs(x = "posterior estimates", y = "", title = "*Anabaena/Cylindrospermum* Mat Anatoxins")

ac_atx_w_c <- ggplot(data = param_est_list[[1]] %>% filter(model == "all_w_cover"), aes(y = parameters_f)) +
  geom_hline(yintercept = (1:5)+0.5, linetype = "dashed", color = "gray") +
  geom_point(aes(x = mean, color = site_reach), position = position_dodge(width = 1)) +
  geom_errorbar(aes(xmin = ci_lower, xmax = ci_upper, color = site_reach),
                position = position_dodge(width = 1)) +
